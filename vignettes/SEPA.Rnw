...
The dataset used to demonstrate the main functions of SEPA is a subset of original gene expression profiles in the HSMMSingleCell package. It contains expressions of 518 genes in 271 for Human Skeletal Muscle Myoblast (HSMM) cells reported in [3]. The 518 genes are experimentally confirmed to be differentially expressed during the cell differentiation process and are used to construct the pseudo-time cell ordering in [3]. The pseudo-time cell orderings are also directly obtained from the HSMMSingleCell package.
